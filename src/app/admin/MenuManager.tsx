...
export default function MenuManager({ sections: initial }: { sections: MenuSectionRow[] }) {
  ...
  return (
    <div className="space-y-8">
      {/* Tab switcher */}
      <div className="flex gap-0 border-b border-gold/15 -mx-2 px-2 overflow-x-auto scrollbar-hide">
        {TABS.map((tab) => (
          <button
            key={tab.id}
      ...
      {visibleSections.length === 0 && (
        <p className="text-cream/40 text-sm">No sections in this menu yet.</p>
      )}

      {visibleSections.map((section) => (
        <div key={section.id}>
          <div className="mb-3">
            <p className="text-[10px] tracking-[0.35em] uppercase text-gold/55">
              {section.subtitle}
              {section.parentSlug && <span className="text-cream/30"> · sub-group of {section.parentSlug}</span>}
            </p>
            <h3 className="font-serif italic text-cream text-xl">{section.label}</h3>
          </div>

          {section.items.length === 0 ? (
            <p className="text-cream/30 text-xs italic">Header section — items live in sub-groups below.</p>
          ) : (
            <>
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border bg-smoke/30 p-3 transition-colors ${
                        item.available ? 'border-gold/10' : 'border-cream/10 opacity-60'
                      }`}
                    >
                      <label className="block text-[9px] tracking-[0.25em] uppercase text-cream/40 mb-1">Name</label>
                      <input
                        value={d.name}
                        onChange={(e) => setField(item.id, 'name', e.target.value)}
                        className="w-full bg-obsidian/40 border border-gold/10 focus:border-gold/40 focus:bg-smoke/60 rounded-md px-2.5 py-2 text-cream/85 font-serif italic text-sm outline-none transition-all"
                      />

                      <label className="block text-[9px] tracking-[0.25em] uppercase text-cream/40 mt-3 mb-1">Description</label>
                      <textarea
                        value={d.description}
                        onChange={(e) => setField(item.id, 'description', e.target.value)}
                        rows={3}
                        className="w-full bg-obsidian/40 border border-gold/10 focus:border-gold/40 focus:bg-smoke/60 rounded-md px-2.5 py-2 text-cream/65 text-xs outline-none transition-all resize-y leading-relaxed"
                      />

                      <div className="flex items-end gap-3 mt-3">
                        <div className="flex-1 min-w-0">
                          <label className="block text-[9px] tracking-[0.25em] uppercase text-cream/40 mb-1">Price (AUD)</label>
                          <div className="relative">
                            {d.price && <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-cream/35 text-xs">$</span>}
                            <input
                              value={d.price}
                              onChange={(e) => setField(item.id, 'price', e.target.value)}
                              placeholder="set-price"
                              inputMode="decimal"
                              className={`w-full bg-obsidian/40 border border-gold/10 focus:border-gold/40 focus:bg-smoke/60 rounded-md ${d.price ? 'pl-5' : 'pl-2.5'} pr-2 py-2 text-cream/80 font-serif italic text-sm outline-none transition-all placeholder:text-cream/25 placeholder:not-italic`}
                            />
                          </div>
                          {item.priceNote && (
                            <p className="text-[9px] tracking-wider uppercase text-cream/30 mt-1 pl-1">{item.priceNote}</p>
                          )}
                        </div>
                        <button
                      ...
                      {dirty && (
                        <button
                          ...
                          className="mt-3 w-full px-4 py-2 rounded-lg bg-gold/15 border border-gold/40 text-gold text-[11px] tracking-[0.2em] uppercase hover:bg-gold/25 transition-colors disabled:opacity-40"
                        >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2 pr-3 w-32">
                            <div className="relative">
                              {d.price && <span className="absolute left-2 top-1/2 -translate-y-1/2 text-cream/30 text-xs">$</span>}
                              <input
                                value={d.price}
                                onChange={(e) => setField(item.id, 'price', e.target.value)}
                                placeholder="set-price"
                                className={`w-full bg-transparent border border-transparent hover:border-gold/15 focus:border-gold/35 focus:bg-smoke/40 rounded-md ${d.price ? 'pl-5' : 'pl-2'} pr-2 py-1 text-cream/70 font-serif italic outline-none transition-all placeholder:text-cream/20 placeholder:not-italic`}
                              />
                            </div>
                            {item.priceNote && (
                              <p className="text-[9px] tracking-wider uppercase text-cream/30 mt-1 pl-2">{item.priceNote}</p>
                            )}
                          </td>
